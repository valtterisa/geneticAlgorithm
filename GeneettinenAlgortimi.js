// @author John Fish 
// @link https://github.com/johnafish/garden/tree/main/geneticracer
// added comments for own sake 
...
class Ball {
    ...
    // piirretään pallo
    draw() {
        this.ctx.fillStyle = "black"; // pallon väri
        if (this.done) {
            this.ctx.fillStyle = "green"; // kun osutaan kohteeseen 
        }
        this.ctx.beginPath();
    ...
    // lasketaan millä pallolla lyhin matka kohteeseen
    calcFitness() {
        var d = Math.sqrt((this.x - 400) ** 2 + (this.y - 765) ** 2); // sqr(x^2+y^2)
...
function loop() {
    if (generation == 2000) { // kierrosten maksimimäärä
        return
    }
    // käydään jokaista geeniä kohden kaikki pallot
    for (let j = 0; j < NUM_GENES; j++) {
        for (let i = 0; i < NUM_BALLS; i++) {
            balls[i].update();
        }
    }
    nextGen() // seuraava generaatio
